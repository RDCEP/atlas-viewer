...
class DataMunger():
  ...
  def grid_to_json(self, var, var2=None, op=np.divide):
    _v, lat, lon = self.grid_to_np(var)
    if var2:
      _v2 = self.grid_to_np(var2)[0]
      _v /= _v2
    new_data = []
    min_lat = np.min(lat)
    min_lon = np.min(lon)
    ...
    with open('../static/json/grid/SA_{}_{}.json'.format(var, self._crop), 'w') as f:
      f.write(
        json.dumps(
          {
            'data': new_data,
